...
$.fn.oCalendar = function(options) {
    ...
    function showModal(str, elem, data){
        ...
        $.ajax({
            ...
            async: false,
            dataType: 'json',
            data: "function=supplier&pid="+id,
            success: function(resp){
                console.log(resp);
                var so = "";
                for(var i=0; i<resp.length; i++){
                    var ev = resp[i];
                    so+="<option value='"+ev.id+"'>"+ev.name+"</option>";
                }
                console.log(so);
                                        ...
                                        <div class="glow">\
                                                <div class="hider">\
                                                    ...
                                                    <div class="mainContainer">\
                                                        <form class="forrm" name="choosesupplier" onsubmit="sendmail();">\
                                                        <input id="cpid" name="cpid" type="hidden" readonly value="'+id+'">\
                                                        Problem ID: '+id+'<br>\
        ...
        else if(data.includes("&isopen=3&")){
            $.ajax({
                url: opts.url,
                type:'POST',
                async: false,
                dataType: 'json',
                data: "function=purposal&pid="+id,
                success: function(resp){
                    console.log(resp);
                                        ...
                                        <div class="modalContainer">\
                                            <div class="glow">\
                                                    <div class="hider">\
                                                        ...
                                                        <div style="clear:both;"></div>\
                                                        <div class="mainContainer">\
                                                        <form class="forrm" name="choosesupplier">\
                                                        <input id="cpid2" name="cpid2" type="hidden" readonly value="'+id+'">\
                                                        Problem ID: '+id+'<br>\
                                                        Supplier name:'+resp.name+'<br>\
                                                        Price (ILS):'+resp.price+'<br>\
                                                        Availability:'+resp.avail+'<br>\
                                                        <button type="submit" value="decline" onclick="return sendmail2(this)">Decline purposal</button><br>\
                                                        <button type="submit" value="confirm" onclick="return sendmail2(this)">Confirm purposal</button></form>\
                                                            </div>\
                                                        </div>\
                                                    </div>\
                                            </div>\
    ...
    function MyEvent(id, ack, date, time, isopen){
        this.id = id;
        this.ack = ack;
        this.date = date;
        this.time = time;
        ...
        if(view == 'month'){
            ...
            if(divSize < 2){
                ...
                let status = "";
                    ...
                    else if(myEvent.isopen == 2){
                        status="orange";
                        ss = "A message was sent to supplier";
                        
                    }
                    else if(myEvent.isopen == 3){
                        status="blue";
                        ss = "The supplier is waiting for confirmation";                        
                    }
                    if(myEvent.isopen == 4){
                        status="green";
                        ss = "Supplier was confirmed";                        
                    }


                html += '<div id="event-'+myEvent.id+'" class="event" style="color:black;background-color:'+status+';">\
                    ...
                    <input name="isopen" type="hidden" value="'+myEvent.isopen+'">\
                ...
                if(elem.find(".monthMore").length == 0){
                    ...
                    html += '<div class="monthMore event" rel="'+elem[0].id+'">expand</div>';
                    elem.append(html);
                } else{
                    return;
                }
    ...
    function jsonToData(json){
        ...
        for(var i=0; i<json.length; i++){
            ...
            var myEvent = new MyEvent(ev.id, ev.ack, ddd, ev.time, ev.isopen);
            addData(myEvent);
        }
    }
                    ...
                    <div class="hider">\
                        ...
                        <div class="modalTableClass">\
                            <table cellspacing="1" cellpadding="0" class="modalTable">\
                                ...
                                <th style="padding-left:5px;">City</th>\
        ...
        $.ajax({
            ...
            success: function(resp){
                for(var i = 0; i<resp.length; i++){
                    ...
                    else if(resp[i].isopen == 2){
                        status="orange";
                        ss = "A message was sent to supplier";
                        
                    }
                    else if(resp[i].isopen == 3){
                        status="blue";
                        ss = "The supplier is waiting for confirmation";                        
                    }
                    if(resp[i].isopen == 4){
                    ...
                    html += '<tr>\
                    <td style="padding:6px;color:black;background-color:'+status+';">'+resp[i].ack+'</td>\
                    <td style="padding:6px;color:black;background-color:'+status+';">'+resp[i].city+'</td>\
                    <td style="padding:6px;color:black;background-color:'+status+';">'+ss+'</td>\
                                <td style="width:50px;">';
                                
                    if(resp[i].isopen == 1 || resp[i].isopen == 3){
                    html +=         '<div class="editEvent" style="margin-right:12px;" rel="'+resp[i].id+'-'+resp[i].ack+'"></div>\
                                    ...
                                    <input name="city" type="hidden" value="'+resp[i].city+'">\
                                    <input name="isopen" type="hidden" value="'+resp[i].isopen+'">\
